...
- **Shipped on `main`:** 3-app → 1-app collapse · Phase 0 (phone-OTP auth + onboarding via `bootstrap_farm` RPC) · Meadow theme + dual light/dark · Super-admin `/admin/[table]` browser · Phase 1A livestock registry · TDD-wired Sentry SDK across all 3 runtimes · TS strict (no `ignoreBuildErrors`) · app-router icon/apple-icon/opengraph-image · storefront 44px-tap-target hardening · hero-trust-strip redesign · CI workflow fixed · biome 0 errors across 128 files · **storefront marketing surface complete**: polished `/` with SVG product illustrations + animated hero sun + 3D card tilt · `/meet-the-farm` brand-defining about page with isometric SVG farm map · `/traceability` explainer + `/trace/[slug]` dynamic QR landing page (JSON-LD Product, sample data, vertical timeline) · `/shop` 10-product catalogue with URL-driven category filter + WhatsApp-order CTA · shared illustrations at `(storefront)/_components/product-illustrations.tsx` · Input primitive bumped to h-11 + text-base (no more iOS zoom)
...
- **Design baseline:** B+ composite, B AI-slop (last full audit at `.design-audit/`, gitignored). All 4 audit HIGH findings closed (FINDING-001/002/003/004). Storefront polish round added CSS/SVG 3D feel without any three.js: animated hero sun, hill silhouette, isometric farm map at `/meet-the-farm`, perspective+rotateX/Y card tilt on hover (motion-safe gated, `prefers-reduced-motion` respected). Net JS added: **0 bytes** (all inline SVG).
- **Known debt:** Shared First Load JS is 218 kB (CLAUDE.md budget is 200 kB) — Sentry + Supabase + next-intl baseline. Needs `@next/bundle-analyzer` pass per Tooling-to-add list.
