  ...
  const rows: AnimalListRow[] = (result.data ?? []).map((row) => {
    ...
    return {
      ...
      current_structure_id:
        row.current_structure_id === null ? null : String(row.current_structure_id),
...
export async function getAnimal(
  ...
  return {
    ...
    dam: damJoin
      ? { id: String(damJoin.id), tag: String(damJoin.tag), name: damJoin.name ?? null }
      : null,
    sire: sireJoin
      ? { id: String(sireJoin.id), tag: String(sireJoin.tag), name: sireJoin.name ?? null }
      : null,
